  ...
  render: function () {
    ...
    return (
      <div className="thumbnail">
        ...
        <a className="btn btn-xs btn-primary" href={`/groups/files/download/${this.props.slug}/${file.id}`}>{i18n.__n('download')}</a>
        <If condition={isAuthorized(me, file)}>
          <Button bsSize='xsmall' bsStyle='danger' onClick={this.onDelete}>{i18n.__n('delete')}</Button>
        </If>
      </div>
    );
  },
